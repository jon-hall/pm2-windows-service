...
While testing this, a few caveats have arisen which need to be detailed upfront, as they can lead to issues when PM2 is installed as a service:
  - The published version of this package currently wants your global `pm2` install to be version `0.15.10` - a fix for this issue is in the works, but it's awaiting thorough testing (see [this issue](https://github.com/jon-hall/pm2-windows-service/issues/4) for more info).
  - If you don't have the `PM2_HOME` enviroment variable set (in such a way that it is available to the service user - the simplest being set it at machine level), you might find PM2 attempting to run in strange directories.  To resolve this, make sure you set the enviorment variable and restart the service - though see the comment just below about the location it points to being accessible for the service user.
