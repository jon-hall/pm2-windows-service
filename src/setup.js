...
function doSetup(answers) {
	...
	const commandPromises = Object.keys(answers)
	...
	return inquirer
		.prompt([
			...
			},
			{
				...
				message:
					'Set PM2_SERVICE_SCRIPTS (the list of start-up scripts for pm2)?'
			},
			{
			...
			},
			{
				...
				message:
					'Set PM2_SERVICE_PM2_DIR (the location of the global pm2 to use with the service)? [recommended]'
			},
			{
				...
				default:
					process.env.PM2_SERVICE_PM2_DIR ||
					common.guess_pm2_global_dir(),
				when(answers) {
